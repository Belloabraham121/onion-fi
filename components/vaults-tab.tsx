...
import { ArrowUpDown, Bot, DollarSign, TrendingUp, Zap, ArrowUp, ArrowDown } from "lucide-react"
...
export function VaultsTab() {
  ...
  const [actionType, setActionType] = useState<"deposit" | "withdraw" | "ai-route">("deposit")
  ...
  const avgAPY = vaults.reduce((sum, vault) => {
    const balance = parseFloat(vault.balance.replace("$", "").replace(",", ""))
    const apy = parseFloat(vault.apy.replace("%", ""))
    return sum + (balance > 0 ? apy : 0)
  }, 0) / vaults.filter(v => parseFloat(v.balance.replace("$", "").replace(",", "")) > 0).length || 0
    ...
    <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className={actionType === "deposit" ? "bg-blue-600 hover:bg-blue-700" : "border-gray-700 text-gray-300 hover:bg-gray-800"}
              >
              ...
              <Button
                ...
                className={actionType === "withdraw" ? "bg-blue-600 hover:bg-blue-700" : "border-gray-700 text-gray-300 hover:bg-gray-800"}
              >
              ...
              <Button
                ...
                className={actionType === "ai-route" ? "bg-purple-600 hover:bg-purple-700" : "border-gray-700 text-gray-300 hover:bg-gray-800"}
              >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <Button
                ...
                disabled={!selectedVault || (actionType === "deposit" && !depositAmount) || (actionType === "withdraw" && !withdrawAmount)}
            ...
            {!selectedVault && (
              <p className="text-sm text-gray-400">Select a vault below to enable actions</p>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gray-900/50 border-gray-800">
        <CardHeader>
          ...
          <p className="text-gray-400">AI-powered vaults that automatically optimize your yield</p>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-800">
                  ...
                  <th className="text-left py-3 px-4 text-gray-400 font-medium">Vault</th>
                  ...
                  <th className="text-left py-3 px-4 text-gray-400 font-medium">Your Balance</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium">Risk</th>
                  <th className="text-left py-3 px-4 text-gray-400 font-medium">AI Score</th>
                  ...
                  >
                    ...
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={`font-semibold ${getAIScoreColor(vault.aiScore)}`}>
                          {vault.aiScore}
                        </span>
                      </div>
                    </td>
                    <td className="py-4 px-4">
                      <span
                        className={`font-medium ${
                          vault.change.startsWith("+") ? "text-green-400" : "text-red-400"
                        }`}
                      >
                        {vault.change}
                      </span>
